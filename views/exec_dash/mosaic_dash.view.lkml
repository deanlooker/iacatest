view: mosaic_dash {
  derived_table: {
    ...
    and f.eventtype_id in (880,878,1590) and a.subs_type='Subscription'
    ...
    left join dm_apalon.dim_dm_application a on f.application_id=a.application_id and f.appid=a.appid
    where f.eventdate between '2019-01-01' and (current_date-2)
    and f.eventtype_id in (880,878) and a.subs_type='Subscription' and a.dm_cobrand not in ('DBA')
    group by grouping sets ((1,2),(1))),
        ...
        from APALON.APALON_BI.SPEND_PROJECTED_FUNNEL s
        where s.date between '2019-01-01' and (current_date-2)
        group by grouping sets ((1,2),(1))),
        ...
        rev as
            ...
            from
              ...
              from (
                     ...
                     --union all select '35' as order_n, null as split, 'Spend Projected' as item, date as date, case when org is null then 'Total' else org end as org,
                     --spend as metric_value, null as installs from spend_proj
  ...
  measure: to_date {
    ...
    sql:
          case when ${item}='Date' or ${item} = 'Bookings' then
            ...
            else concat(to_char(sum(case when ${TABLE}.date >= date_trunc(month,dateadd(day,-2,current_date())) then coalesce(${TABLE}.metric_value,0) else 0 end)/1000,'$999,999,990'),'k') end ;;
    ...
    sql:case when ${item} in ('Total Gross Bookings','Spend') and ${data_check}<dateadd(day,-2,current_date()) then 'No Data Yet' else
          case when ${item}='Date' or ${item} = 'Bookings' then concat(cast(month(dateadd(day,-8,current_date())) as varchar(2)),'/',cast(day(dateadd(day,-8,current_date())) as varchar(2)),
            ...
            when ${item} like ('%CVR%') then concat(to_char(sum(case when ${TABLE}.date between current_date()-8 and current_date()-2 then coalesce(${TABLE}.metric_value,0) else 0 end)/
              ...
              else concat(to_char(sum(case when ${TABLE}.date between current_date()-8 and current_date()-2 then coalesce(${TABLE}.metric_value,0) else 0 end)/1000,'$999,999,999'),'k') end end;;
    html:   {% if item._rendered_value == '_' %}
          <div style="color: #f0f0f0; font-weight: bold; font-size:100%; text-align:center; background-color:#f0f0f0">{{ rendered_value }}</div>
          {% elsif item._rendered_value == 'Bookings' %}
          {% if business._rendered_value == "Apalon" %}
  ...
  measure: fc {
    ...
    label: "{{ mosaic_dash.forecast_date._value }}"
