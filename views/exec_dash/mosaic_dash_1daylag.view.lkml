view: mosaic_dash_1daylag {
  derived_table: {
    sql:
      ...
      where f.eventdate between '2019-01-01' and (current_date-1)
      ...
      sum(case when e.event in ('Crossgrade',
          ...
          from APALON.ERC_APALON.APPLE_SUBSCRIPTION_EVENT e
              ...
              where e.date between '2019-01-01' and (current_date-1)
      ...
      where g.date between '2019-01-01' and (current_date-1)
      ...
      where r.product_type_identifier in ('App', 'App Universal','App iPad','App Mac','App Bundle')
                  and r.begin_date between '2019-01-01' and (current_date-1)
      ...
      --sum(case when (f.payment_number=1 and length(f.SUBSCRIPTION_LENGTH )=3) OR (f.payment_number=0 and datediff(day,to_date(f.dl_date),to_date(f.original_purchase_date))=0) then f.subscriptionpurchases else 0 end) as acquisitions
      ...
      ,aCVR as (
      select a.date,a.org,sum(a.trials) trials,sum(a.new_subscribers_direct) new_subscribers_direct,sum(a.trials + a.new_subscribers_direct) acquisitions
      ,sum(b.installs) installs

      from (

                  select
                  date,company org,sum(new_trials) trials
                  ,sum(new_subscribers_direct) new_subscribers_direct
                  from ${kpi_subs_report.SQL_TABLE_NAME}
                  where contains(app,'iOS')
                  group by 1,2
                  union all
                  select
                  date,company org,sum(trials) trials,0 new_subscribers_direct
                  from ${kpi_ua_report.SQL_TABLE_NAME}
                  where true
                  and (contains(app,'iOS') = False -- apparently google trials come from kpi_ua.sql
                  or app = 'TrapCall iOS')
                  group by 1,2
                  ) a
      left join data b on a.date = b.date and a.org = b.org
      group by grouping sets ((1,2),(1))
      )
          ...
          ,rev as
              ...
              sum(case when ft.fact_type='app' then f.gross_proceeds
                    ...
                    when (ft.fact_type='ad' and a.app_family_name='Translation' and f.date<'2019-08-01') then f.ad_revenue*1.009487
                    when (ft.fact_type='ad' and a.app_family_name='Translation' and f.date<'2019-09-01') then f.ad_revenue*1.019424
                    when (ft.fact_type='ad' and a.app_family_name='Translation' and f.date<'2019-10-01') then f.ad_revenue*1.013585
                    when (ft.fact_type='ad' and a.app_family_name='Translation' and f.date<'2019-11-01') then f.ad_revenue*1.019894
                    when (ft.fact_type='ad' and a.app_family_name='Translation' and f.date<'2019-12-01') then f.ad_revenue*1.020092
                    when (ft.fact_type='ad' and a.app_family_name='Translation' and f.date<'2020-01-01') then f.ad_revenue*1.024119
                    when (ft.fact_type='ad' and a.app_family_name='Translation' and f.date>='2020-01-01') then f.ad_revenue*1.019444
                    ...
                    when (ft.fact_type='ad' and a.org='apalon' and f.date<'2019-08-01') then f.ad_revenue*1.135073
                    when (ft.fact_type='ad' and a.org='apalon' and f.date<'2019-09-01') then f.ad_revenue*1.146405
                    when (ft.fact_type='ad' and a.org='apalon' and f.date<'2019-10-01') then f.ad_revenue*1.150299
                    when (ft.fact_type='ad' and a.org='apalon' and f.date<'2019-11-01') then f.ad_revenue*1.151567
                    when (ft.fact_type='ad' and a.org='apalon' and f.date<'2019-12-01') then f.ad_revenue*1.145559
                    when (ft.fact_type='ad' and a.org='apalon' and f.date<'2020-01-01') then f.ad_revenue*1.143447
                    when (ft.fact_type='ad' and a.org='apalon' and f.date>='2020-01-01') then f.ad_revenue*1.10665
              ...
              from ERC_APALON.FACT_REVENUE f
               ...
               and f.date between '2019-01-01' and (current_date-1)
               --and a.cobrand<>'DAQ'
              ...
              and date between '2019-01-01' and (current_date-1)
              group by 1,2)
                     ...
                     d0_trials as metric_value, installs as installs from data
                     union all select '80' as order_n, null as split, 'Acquisitions' as item, date as date, case when org is null then 'Total' else org end as org,
                     acquisitions as metric_value, installs as installs from aCVR
                     union all select '90' as order_n, null as split, 'aCVR' as item, date as date, case when org is null then 'Total' else org end as org,
                     acquisitions as metric_value, installs as installs from aCVR
                     union all select '91' as order_n, null as split, 'pCVR*' as item, date as date, org as org,
                     sum(first_purchases) as metric_value, sum(store_installs) as installs from pcvr group by 4,5
                     union all select '91' as order_n, null as split, 'pCVR*' as item, date as date, 'Total' as org,
  ...
  measure: to_date {
    ...
    label: "Month to Date"
    type: string
    sql:case when ${item} in ('Total Gross Bookings','Spend','pCVR*') and
          sum(case when ${TABLE}.date=current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)<sum(case when ${TABLE}.date=current_date()-3 then coalesce(${TABLE}.metric_value*0.5,0) else 0 end)
          or ${item} ='pCVR*' and sum(case when ${TABLE}.date=current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)>sum(case when ${TABLE}.date=current_date()-3 then coalesce(${TABLE}.metric_value*1.65,0) else 0 end) then 'No Last Day Data' else
          --case when ${data_check}<dateadd(day,-1,current_date()) then 'No Data Yet' else
                case when ${item}='Date' or ${item} = 'Bookings' then
                  (case when date_trunc(month,current_date())>dateadd(day,-1,current_date()) then 'No Data'
                  else  concat(cast(month(date_trunc(month,current_date())) as varchar(2)),'/',cast(day(date_trunc(month,current_date())) as varchar(2)),' - ',cast(month(dateadd(day,-1,current_date())) as varchar(2)),'/',cast(day(dateadd(day,-1,current_date())) as varchar(2))) end)
                  ...
                  when ${item}='Installs' or ${item}='D0 Trials' or ${item}='Acquisitions' then to_char(sum(case when ${TABLE}.date >= date_trunc(month,current_date()) then coalesce(${TABLE}.metric_value,0) else 0 end),'999,999,990')
  ...
  measure: last_month_to_date {
    ...
    sql: case when ${item}='Date' or ${item} = 'Bookings' then
      (case when date_trunc(month,current_date())>dateadd(day,-1,current_date()) then 'No Data'
              ...
              ' - ',cast(month(dateadd(month,-1,dateadd(day,-1,current_date()))) as varchar(2)),'/',cast(day(dateadd(month,-1,dateadd(day,-1,current_date()))) as varchar(2))) end)
      when ${item} like ('%CVR%') then concat(to_char(sum(case when ${TABLE}.date >= date_trunc(month,dateadd(month,-1,current_date())) and ${TABLE}.date <= dateadd(month,-1,dateadd(day,-1,current_date())) then coalesce(${TABLE}.metric_value,0) else 0 end)/nullif(sum(case when ${TABLE}.date >= date_trunc(month,dateadd(month,-1,current_date())) and ${TABLE}.date <= dateadd(month,-1,dateadd(day,-1,current_date())) then coalesce(${TABLE}.installs,0) else 0 end),0)*100,'999,990D00'),'%')
      when sum(case when ${TABLE}.date >= date_trunc(month,dateadd(month,-1,current_date())) and ${TABLE}.date <= dateadd(month,-1,dateadd(day,-1,current_date())) then coalesce(${TABLE}.metric_value,0) else 0 end)=0 then '-'
      when ${item}='Installs' or ${item}='D0 Trials' or ${item}='Acquisitions' then to_char(sum(case when ${TABLE}.date >= date_trunc(month,dateadd(month,-1,current_date())) and ${TABLE}.date <= dateadd(month,-1,dateadd(day,-1,current_date())) then coalesce(${TABLE}.metric_value,0) else 0 end),'999,999,990')
      else concat(to_char(sum(case when ${TABLE}.date >= date_trunc(month,dateadd(month,-1,current_date())) and ${TABLE}.date <= dateadd(month,-1,dateadd(day,-1,current_date())) then coalesce(${TABLE}.metric_value,0) else 0 end)/1000,'$999,999,990'),'k') end;;
    html:   {% if item._rendered_value == '_' %}
          <div style="color: #f0f0f0; font-weight: bold; font-size:100%; text-align:center; background-color:#f0f0f0">{{ rendered_value }}</div>
          {% elsif item._rendered_value == 'Bookings' %}
          {% if business._rendered_value == "Apalon" %}
  ...
  measure: 8d_ago {
    description: "Data 8 days ago"
    label: "8d Ago"
    type: string
    sql: case when ${item}='Date' or ${item} = 'Bookings' then concat(cast(month(dateadd(day,-8,current_date())) as varchar(2)),'/',cast(day(dateadd(day,-8,current_date())) as varchar(2)))
      when ${item} like ('%CVR%') then concat(to_char(sum(case when ${TABLE}.date=current_date()-8 then coalesce(${TABLE}.metric_value,0) else 0 end)/
        nullif(sum(case when ${TABLE}.date=current_date()-8 then coalesce(${TABLE}.installs,0) else 0 end),0)*100,'999,990D00'),'%')
        when sum(case when ${TABLE}.date=current_date()-8 then coalesce(${TABLE}.metric_value,0) else 0 end)=0 then '-'
        when ${item}='Installs' or ${item}='D0 Trials' or ${item}='Acquisitions' then to_char(sum(case when ${TABLE}.date=current_date()-8 then coalesce(${TABLE}.metric_value,0) else 0 end),'999,999,990')
        else concat(to_char(sum(case when ${TABLE}.date=current_date()-8 then coalesce(${TABLE}.metric_value,0) else 0 end)/1000,'$999,999,990'),'k') end;;
    html:   {% if item._rendered_value == '_' %}
          <div style="color: #f0f0f0; font-weight: bold; font-size:100%; text-align:center; background-color:#f0f0f0">{{ rendered_value }}</div>
          {% elsif item._rendered_value == 'Bookings' %}
          {% if business._rendered_value == "Apalon" %}
  ...
  measure: 1d_ago {
    ...
    sql: case when ${item} in ('Total Gross Bookings','Spend','pCVR*') and
          ...
          or ${item} ='pCVR*' and sum(case when ${TABLE}.date=current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)>=sum(case when ${TABLE}.date=current_date()-3 then coalesce(${TABLE}.metric_value*1.65,0) else 0 end) then 'No Last Day Data' else
     --case when ${data_check}<dateadd(day,-1,current_date()) then 'No Data Yet' else
              case when ${item}='Date' or ${item} = 'Bookings' then concat(cast(month(dateadd(day,-1,current_date())) as varchar(2)),'/',cast(day(dateadd(day,-1,current_date())) as varchar(2)))
               when ${item} like ('%CVR%') then concat(to_char(sum(case when ${TABLE}.date=current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)/
               nullif(sum(case when ${TABLE}.date=current_date()-1 then coalesce(${TABLE}.installs,0) else 0 end),0)*100,'999,990D00'),'%')
               when sum(case when ${TABLE}.date=current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)=0 then '-'
               when ${item}='Installs' or ${item}='D0 Trials' or ${item}='Acquisitions' then to_char(sum(case when ${TABLE}.date=current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end),'999,999,990')
               else concat(to_char(sum(case when ${TABLE}.date=current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)/1000,'$999,999,990'),'k') end end;;
    html:   {% if item._rendered_value == '_' %}
          <div style="color: #f0f0f0; font-weight: bold; font-size:100%; text-align:center; background-color:#f0f0f0">{{ rendered_value }}</div>
          {% elsif item._rendered_value == 'Bookings' %}
          {% if business._rendered_value == "Apalon" %}
  ...
  measure: wtd {
    ...
    sql:case when ${item} in ('Total Gross Bookings','Spend','pCVR*') and
          ...
          or ${item} ='pCVR*' and sum(case when ${TABLE}.date=current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)>sum(case when ${TABLE}.date=current_date()-3 then coalesce(${TABLE}.metric_value*1.65,0) else 0 end) then 'No Last Day Data' else
          --case when ${data_check}<dateadd(day,-1,current_date()) then 'No Data Yet' else
                case when datediff(day,dateadd(day,-1,date_trunc(week,dateadd(day,-1,current_date()))),dateadd(day,-1,current_date()))=7 then ${1d_ago}
                when ${item}='Date' or ${item} = 'Bookings' then
                concat(cast(month(dateadd(day,-1,date_trunc(week,dateadd(day,-1,current_date())))) as varchar(2)),'/',cast(day(dateadd(day,-1,date_trunc(week,dateadd(day,-1,current_date())))) as varchar(2)),
                          ' - ',cast(month(dateadd(day,-1,current_date())) as varchar(2)),'/',cast(day(dateadd(day,-1,current_date())) as varchar(2)))
                  when ${item} like ('%CVR%') then concat(to_char(sum(case when ${TABLE}.date between dateadd(day,-1,date_trunc(week,dateadd(day,-1,current_date()))) and current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)/
                    nullif(sum(case when ${TABLE}.date between dateadd(day,-1,date_trunc(week,dateadd(day,-1,current_date()))) and current_date()-1 then coalesce(${TABLE}.installs,0) else 0 end),0)*100,'999,990D00'),'%')
                    when sum(case when ${TABLE}.date between dateadd(day,-1,date_trunc(week,dateadd(day,-1,current_date()))) and current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)=0 then '-'
                    when ${item}='Installs' or ${item}='D0 Trials' or ${item}='Acquisitions' then to_char(sum(case when ${TABLE}.date between dateadd(day,-1,date_trunc(week,dateadd(day,-1,current_date()))) and current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end),'999,999,990')
                    else concat(to_char(sum(case when ${TABLE}.date between dateadd(day,-1,date_trunc(week,dateadd(day,-1,current_date()))) and current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)/1000,'$999,999,990'),'k') end end;;
    html:   {% if item._rendered_value == '_' %}
          <div style="color: #f0f0f0; font-weight: bold; font-size:100%; text-align:center; background-color:#f0f0f0">{{ rendered_value }}</div>
          {% elsif item._rendered_value == 'Bookings' %}
          {% if business._rendered_value == "Apalon" %}
  ...
  measure: last_7d {
    ...
    label: "Last 7d"
    type: string
    sql:case when ${data_check}<dateadd(day,-1,current_date()) then 'No Data Yet' else
          case when ${item}='Date' or ${item} = 'Bookings' then concat(cast(month(dateadd(day,-7,current_date())) as varchar(2)),'/',cast(day(dateadd(day,-7,current_date())) as varchar(2)),
                    ' - ',cast(month(dateadd(day,-1,current_date())) as varchar(2)),'/',cast(day(dateadd(day,-1,current_date())) as varchar(2)))
            when ${item} like ('%CVR%') then concat(to_char(sum(case when ${TABLE}.date between current_date()-7 and current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)/
              nullif(sum(case when ${TABLE}.date between current_date()-7 and current_date()-1 then coalesce(${TABLE}.installs,0) else 0 end),0)*100,'999,990D00'),'%')
              when sum(case when ${TABLE}.date between current_date()-7 and current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)=0 then '-'
              when ${item}='Installs' or ${item}='D0 Trials' or ${item}='Acquisitions' then to_char(sum(case when ${TABLE}.date between current_date()-7 and current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end),'999,999,990')
              else concat(to_char(sum(case when ${TABLE}.date between current_date()-7 and current_date()-1 then coalesce(${TABLE}.metric_value,0) else 0 end)/1000,'$999,999,990'),'k') end end;;
    html:   {% if item._rendered_value == '_' %}
          <div style="color: #f0f0f0; font-weight: bold; font-size:100%; text-align:center; background-color:#f0f0f0">{{ rendered_value }}</div>
          {% elsif item._rendered_value == 'Bookings' %}
          {% if business._rendered_value == "Apalon" %}
  ...
  measure: prev_7d {
    ...
    type: string
    sql: case when ${item}='Date' or ${item} = 'Bookings' then concat(cast(month(dateadd(day,-14,current_date())) as varchar(2)),'/',cast(day(dateadd(day,-14,current_date())) as varchar(2)),
              ' - ',cast(month(dateadd(day,-8,current_date())) as varchar(2)),'/',cast(day(dateadd(day,-8,current_date())) as varchar(2)))
      when ${item} like ('%CVR%') then concat(to_char(sum(case when ${TABLE}.date between current_date()-14 and current_date()-8 then coalesce(${TABLE}.metric_value,0) else 0 end)/
        nullif(sum(case when ${TABLE}.date between current_date()-14 and current_date()-8 then coalesce(${TABLE}.installs,0) else 0 end),0)*100,'999,990D00'),'%')
        when sum(case when ${TABLE}.date between current_date()-14 and current_date()-8 then coalesce(${TABLE}.metric_value,0) else 0 end)=0 then '-'
        when ${item}='Installs' or ${item}='D0 Trials' or ${item}='Acquisitions' then to_char(sum(case when ${TABLE}.date between current_date()-14 and current_date()-8 then coalesce(${TABLE}.metric_value,0) else 0 end),'999,999,990')
        else concat(to_char(sum(case when ${TABLE}.date between current_date()-14 and current_date()-8 then coalesce(${TABLE}.metric_value,0) else 0 end)/1000,'$999,999,990'),'k') end;;
    html:   {% if item._rendered_value == '_' %}
          <div style="color: #f0f0f0; font-weight: bold; font-size:100%; text-align:center; background-color:#f0f0f0">{{ rendered_value }}</div>
          {% elsif item._rendered_value == 'Bookings' %}
          {% if business._rendered_value == "Apalon" %}
  ...
  measure: wtd_prev {
    ...
    label: "WTD previous"
    type: string
    sql: case when datediff(day,dateadd(day,-1,date_trunc(week,dateadd(day,-1,current_date()))),dateadd(day,-1,current_date()))=7 then ${8d_ago}
          when ${item}='Date' or ${item} = 'Bookings' then concat(cast(month(dateadd(day,-7,date_trunc(week,dateadd(day,-1,current_date())))) as varchar(2)),'/',cast(day(dateadd(day,-8,date_trunc(week,dateadd(day,-1,current_date())))) as varchar(2)),
                    ' - ',cast(month(dateadd(day,-8,current_date())) as varchar(2)),'/',cast(day(dateadd(day,-8,current_date())) as varchar(2)))
            when ${item} like ('%CVR%') then concat(to_char(sum(case when ${TABLE}.date between dateadd(day,-7,date_trunc(week,dateadd(day,-1,current_date()))) and current_date()-8 then coalesce(${TABLE}.metric_value,0) else 0 end)/
              nullif(sum(case when ${TABLE}.date between dateadd(day,-7,date_trunc(week,dateadd(day,-1,current_date()))) and current_date()-8 then coalesce(${TABLE}.installs,0) else 0 end),0)*100,'999,990D00'),'%')
              when sum(case when ${TABLE}.date between dateadd(day,-7,date_trunc(week,dateadd(day,-1,current_date()))) and current_date()-8 then coalesce(${TABLE}.metric_value,0) else 0 end)=0 then '-'
              when ${item}='Installs' or ${item}='D0 Trials' or ${item}='Acquisitions' then to_char(sum(case when ${TABLE}.date between dateadd(day,-7,date_trunc(week,dateadd(day,-1,current_date()))) and current_date()-8 then coalesce(${TABLE}.metric_value,0) else 0 end),'999,999,990')
              else concat(to_char(sum(case when ${TABLE}.date between dateadd(day,-7,date_trunc(week,dateadd(day,-1,current_date()))) and current_date()-8 then coalesce(${TABLE}.metric_value,0) else 0 end)/1000,'$999,999,990'),'k') end;;
    html:   {% if item._rendered_value == '_' %}
          <div style="color: #f0f0f0; font-weight: bold; font-size:100%; text-align:center; background-color:#f0f0f0">{{ rendered_value }}</div>
          {% elsif item._rendered_value == 'Bookings' %}
          {% if business._rendered_value == "Apalon" %}
  ...
  measure: rr {
    ...
    type: number
    sql:
    case when ${item}='Date' or ${item} = 'Bookings' then 0
    when ${item} in ('Total Gross Bookings','Spend') then
    (sum(case when ${TABLE}.date between dateadd(day,-6,${data_check}) and ${data_check} then coalesce(${TABLE}.metric_value,0) else 0 end)/7
            *datediff(day,(${data_check}),date_trunc(month,dateadd(month,1,current_date()))-1)+sum(case when ${TABLE}.date<=${data_check} and ${TABLE}.date >= date_trunc(month,current_date()) then coalesce(${TABLE}.metric_value,0) else 0 end))
    else
        case when ${item} like ('%CVR%') then (sum(case when ${TABLE}.date between dateadd(day,-6,${day_ago}) and ${day_ago} then coalesce(${TABLE}.metric_value,0) else 0 end)/7
            *datediff(day,(${day_ago}),date_trunc(month,dateadd(month,1,current_date()))-1)+sum(case when ${TABLE}.date<=${day_ago} and ${TABLE}.date >= date_trunc(month,current_date()) then coalesce(${TABLE}.metric_value,0) else 0 end))/nullif((sum(case when ${TABLE}.date between dateadd(day,-6,${day_ago}) and ${day_ago} then coalesce(${TABLE}.installs,0) else 0 end)/7
            *datediff(day,(${day_ago}),date_trunc(month,dateadd(month,1,current_date()))-1)+sum(case when ${TABLE}.date<=${day_ago} and ${TABLE}.date >= date_trunc(month,current_date()) then coalesce(${TABLE}.installs,0) else 0 end)),0)*100
            else
             (sum(case when ${TABLE}.date between dateadd(day,-6,${day_ago}) and ${day_ago} then coalesce(${TABLE}.metric_value,0) else 0 end)/7
            *datediff(day,(${day_ago}),date_trunc(month,dateadd(month,1,current_date()))-1)+sum(case when ${TABLE}.date<=${day_ago} and ${TABLE}.date >= date_trunc(month,current_date()) then coalesce(${TABLE}.metric_value,0) else 0 end))
        end
    end;;
  }
  ...
  measure: run_rate {
    ...
    case when ${item}='Date' or ${item} = 'Bookings' then concat(cast(month(date_trunc(month,current_date())) as varchar(2)),'/',cast(day(date_trunc(month,current_date())) as varchar(2)),
      ...
      when ${rr}=0 then '-'
      when ${item}='Installs' or ${item}='D0 Trials' or ${item}='Acquisitions' then to_char(${rr},'999,999,990')
  ...
  measure: prev_mon {
    ...
    sql: case when ${item}='Date' or ${item} = 'Bookings' then concat(cast(month(dateadd(month,-1,date_trunc(month,current_date()))) as varchar(2)),'/',cast(day(dateadd(month,-1,date_trunc(month,current_date()))) as varchar(2)),
            ...
            when ${item}='Installs' or ${item}='D0 Trials' or ${item} = 'Acquisitions' then to_char(${pm},'999,999,990')
  ...
  measure: fc {
    ...
    sql:  case when ${item}='Date' or ${item} = 'Bookings' then concat(cast(month(date_trunc(month,current_date())) as varchar(2)),'/',cast(day(date_trunc(month,current_date())) as varchar(2)),
            ...
            when ${item}='Installs' or ${item}='D0 Trials' or ${item}='Acquisitions' then to_char(${forecast},'999,999,990')
  ...
  measure: diff {
    ...
    case when ${item}='Date' or ${item} = 'Bookings' then concat(cast(month(date_trunc(month,current_date())) as varchar(2)),'/',cast(day(date_trunc(month,current_date())) as varchar(2)),
          ...
          when ${item}='Installs' or ${item}='D0 Trials' or ${item}='Acquisitions' then to_char(${vs},'999,999,990')
...
#     description: "Last Available Date per Business (1D lag)"
#     #hidden: yes
#     type: date
#     sql: ${business_lvl_data_check.latest_date_1dbefore};;
#   }
  ...
  dimension: data_check {
    ...
    #hidden: yes
    type: date
    sql: coalesce(${exec_dash_date_check.latest_date_1dbefore},current_date()-1);;
  }

  dimension: day_ago {
    description: "1 Day Ago Date"
    #hidden: yes
    type: date
    sql:current_date()-1;;
  }
