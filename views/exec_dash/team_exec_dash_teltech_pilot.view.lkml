view: team_exec_dash_teltech_pivot {
  ...
  derived_table: {
    sql:with metric_description as
      ...
      (SELECT date,
          ...
          JOIN APALON.ERC_APALON.DIM_APP a  ON f.APP_ID = a.APP_ID and a.org='TelTech'
      ...
      from apalon.dm_apalon.fact_global f
        ...
        join (select distinct application_id from dm_apalon.dim_dm_application where subs_type='Subscription' and org='TelTech' ) da  on  da.application_id=f.application_id
      ...
      from apalon.dm_apalon.fact_global f
       join calend
         join dm_apalon.dim_dm_application da on da.subs_type='Subscription' and da.application_id=f.application_id and da.org='TelTech'
      ...
      where f.eventtype_id=880  and payment_number>0  -- subscriptions
           ...
           (select 1 from apalon.dm_apalon.fact_global c
            where c.eventtype_id=1590 and
                  c.application_id=f.application_id and c.uniqueuserid=f.uniqueuserid and
                  c.transaction_id=f.transaction_id and c.subscription_expiration_date<dago)
      GROUP  BY 1, 2,3
      union all
      select dago as date, NULL as geo,
            19 as metric_id,
      ...
      where f.eventtype_id=880  and payment_number>0 -- subscriptions
           ...
           (select 1 from apalon.dm_apalon.fact_global c
            where c.eventtype_id=1590 and
                  c.application_id=f.application_id and c.uniqueuserid=f.uniqueuserid and
                  c.transaction_id=f.transaction_id and c.subscription_expiration_date<dateadd(day,-30,dago))
      GROUP  BY 1, 2,3
       )
        ...
        select case when metric='revenue'  then
               ...
               decode(extract('month',current_date()),
                 ...
                 12, ' Dec FC of ')||to_varchar(max(forecast_date),'MM/DD/YY') as period,5 as period_order,'x' as geo, sum(forecast)  as metric_value
                                from REPORTS_SCHEMA.EXECUTIVE_DASHBOARD_FORECAST_BU where month=date_part('month',current_date) and BUSINESS_UNIT='TelTech' group by 1
       ) m
