...
where  f.dl_date>='2018-09-01' and f.eventdate>='2018-09-01'
...
where  f.dl_date>='2018-09-01' and f.eventdate>='2018-09-01' and f.eventtype_id in (1297,878,1590)
    and f.APPLICATION_ID in(select da.APPLICATION_ID from dm_apalon.dim_dm_application da where da.SUBS_TYPE='Subscription')
  ...
  measure: Retention_D7  {
    hidden: yes
    label: "Retention D7"
    type: number
    value_format: "0.00%"
    sql:  count(distinct case when DATEDIFF(day,to_date(${TABLE}.dl_date),to_date(${TABLE}.eventdate)) = 7
                     AND ${TABLE}.EVENTTYPE_ID=1297 and ${TABLE}.cancelled_or_not=0 THEN ${TABLE}.UNIQUEUSERID else null end)/${INSTALLS};;
  ...
  measure: d1_retention {
    ...
    type: number
    value_format: "0.00%"
    sql:  case when DATEDIFF(day,to_date(${TABLE}.dl_date),current_date())<3 then null else count(distinct case when DATEDIFF(day,to_date(${TABLE}.dl_date),to_date(${TABLE}.eventdate))=1
      AND ${TABLE}.EVENTTYPE_ID=1297 THEN ${TABLE}.UNIQUEUSERID else null end)/${INSTALLS} end;;
  }
  ...
  measure: d7_retention {
    ...
    type: number
    value_format: "0.00%"
    sql: case when DATEDIFF(day,to_date(${TABLE}.dl_date),current_date())<9 then null else count(distinct case when DATEDIFF(day,to_date(${TABLE}.dl_date),to_date(${TABLE}.eventdate))=7
      AND ${TABLE}.EVENTTYPE_ID=1297 THEN ${TABLE}.UNIQUEUSERID else null end)/${INSTALLS} end;;
  }
  ...
  measure: d30_retention {
    ...
    type: number
    value_format: "0.00%"
    sql: case when DATEDIFF(day,to_date(${TABLE}.dl_date),current_date())<32 then null else count(distinct case when DATEDIFF(day,to_date(${TABLE}.dl_date),to_date(${TABLE}.eventdate))=30
      AND ${TABLE}.EVENTTYPE_ID=1297 THEN ${TABLE}.UNIQUEUSERID else null end)/${INSTALLS} end;;
  }
  ...
  measure: d60_retention {
    ...
    type: number
    value_format: "0.00%"
    sql: case when DATEDIFF(day,to_date(${TABLE}.dl_date),current_date())<62 then null else count(distinct case when DATEDIFF(day,to_date(${TABLE}.dl_date),to_date(${TABLE}.eventdate))=60
      AND ${TABLE}.EVENTTYPE_ID=1297 THEN ${TABLE}.UNIQUEUSERID else null end)/${INSTALLS} end;;
  }

  measure: SessionEvent_Count {
    hidden: no
    description: "Count of SessionEvent events performed by Subscribers in the last 7 days"
    label: "SessionEvent Count"
    type: sum
    sql: case when ${TABLE}.EVENTTYPE_ID=1297 and ${TABLE}.cancelled_or_not=0 and DATEDIFF(day,current_date(),to_date(${TABLE}.eventdate)) <=7
          then 1  end;;
  }

  measure: Subscribers_Count {
    hidden: no
    description: "Count of Subscribers in the last 7 days"
