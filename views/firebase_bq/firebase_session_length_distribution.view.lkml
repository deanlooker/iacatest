view: firebase_session_length_distribution {
  ...
  derived_table: {
    sql:
      ...
      WHERE session_duration < CASE WHEN CAST(1000 / {% parameter bucket_size %} AS INT64) = (1000 / {% parameter bucket_size %}) THEN 1000
          ELSE {% parameter bucket_size %} * (CAST(1000 / {% parameter bucket_size %} AS INT64) + 1) END
      GROUP BY 1
      ;;
  }
  ...
  dimension: dynamic_duration  {
    sql:
        concat(CAST(${session_duration} - mod(${session_duration},{% parameter bucket_size %}) AS string),
          ' - ', CAST(${session_duration} - mod(${session_duration},{% parameter bucket_size %}) + {% parameter bucket_size %}  AS string))
      ;;
  ...
  dimension: dynamic_sort_field {
    sql:
      ${session_duration} - mod(${session_duration},{% parameter bucket_size %});;
    type: number
    hidden: yes
  }
