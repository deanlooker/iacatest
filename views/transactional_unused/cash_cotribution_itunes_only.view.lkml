view: cash_contribution_itunes_only {
  derived_table:
...
-- select *
-- from data where account='apalon' and cobrand ='BUL'
----------------------------------------------------------------------------------------------------------------------------------------------------------------
 ,spend as(
   SELECT  CASE WHEN a.app_family_name = 'Translation' THEN 'iTranslate' ELSE a.org END AS organization
    ...
    WHERE m.eventdate >= '2019-03-01' --and cobrand='BUL'
...
left join  APALON.DM_APALON.DIM_DM_APPLICATION ap on lower(trim(ap.UNIFIED_NAME)) = lower(trim (case
               ...
               where lower("App name") not like ('%snap%trans%') and date<'2019-03-01' --and ap.dm_cobrand='CFL'
...
group by 1,2,3,4,5 order by 7 asc
 ) --select * from spend where cobrand='BUL'
 ...
 (select  d.osd as date
        ...
        ,case when abs(datediff(day,current_date(),date_trunc(month,current_date())))<=7 and lower(ORGANIZATION)not  in ('teltech','itranslate')
 ...
 left join spend s on s.cobrand=d.cobrand and s.platform=d.platform and d.osd=s.date
        ...
        where sbg.run_date=(select max(run_date) from apalon.apalon_bi.curves_sbg) and s.check_id=1 --and d.cobrand='DAX'
                and (d.account  in ('apalon','dailyburn') and d.cobrand not in ('CWK','C5I','BUS','BUT','C0M'))
...
group by 1,2,3,4,5) --select * from proj_pcvr
...
where lower(p.ORGANIZATION) in ('apalon','dailyburn')  ) --select * from mtd
...
from result
where date_trunc(month,date)=date_trunc(month,date_trunc(month,current_date())-34) and ORGANIZATION in ('apalon','dailyburn')
...
-----------------------------&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
 ,final_result AS (
  ...
  UNION
   ...
   0 as NET_PRICE,0 as GROSS_PRICE,0 as LIFETIME,0 as PROJECTED_REVENUE,0 as SPEND_TOTAL,0 as AVG_SPEND,t.BEGIN_M,t.DDRR
  ...
  dimension_group: date {
    ...
    dimension: filter_on_app_name_apalon {
      ...
      type: string
      suggestions: ["Weather Live Free","Noaa Weather Radar Free",
        "Coloring Book for Me Free","Live Wallpapers Free","Scanner for Me Free","Wallpapers for Me Free",
        "Productive App","Planes Live Flight Tracker Free","Sleepzy","Snap Calc Free",
        "VPN Free","Jigsaw Puzzles for Me Sub","Super Pixel","Zodiask","Cycle Tracker","Eggzy",
        "Voice Recorder for Me Sub","Photo Scanner for Me (Free)"
      ]
      sql: ${TABLE}.unified_name ;;
  ...
  measure: trials_mtd {
    description: "Trials MTD"
    label:"Trials MTD"
    type: sum
    value_format: "#,##0"
    sql: ${TABLE}.trials_mtd ;;
  }
